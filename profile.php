...
$posts = mysqli_query($db_con, "SELECT COUNT(*) FROM `blogs` WHERE `user_id`='$id'");
$count_post = mysqli_fetch_assoc($posts);
// Likes
$likes = mysqli_query($db_con, "SELECT COUNT(*) FROM `likes` WHERE `user_id`='$id'");
$count_like = mysqli_fetch_assoc($likes);
// Comment
$commentss = mysqli_query($db_con, "SELECT COUNT(*) FROM `comment` WHERE `user_id`='$id'");
...
if(isset($_POST['updateProfileBtn'])){
  ...
  $updateProfilequery = "UPDATE `users` SET `f_name`='$f_name',`l_name`='$l_name',`city`='$city',`country`='$country',`gender`='$gender',`birth`='$birth',`addr`='$addr',`institute`='$institute',`user_type`='$user_type' WHERE `id` = '$id'";
...
if(isset($_POST['updateProfilePictureBtn'])){
  ...
  header('Location: profile.php?profilePicture_updated');
}
?>
...
<div class="modal fade" id="updateProfile" tabindex="-1" aria-labelledby="updatePictureLabel" aria-hidden="true">
